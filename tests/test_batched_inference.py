...
def test_batched_kinematics_and_loss():
    ...
    # Wrap the function in a closure so vmap doesn't complain about the dicts
    ...
    # --- FIX: CALL batched_loss_fn INSTEAD OF decoupled_crystallographic_loss_pure ---
    loss = batched_loss_fn(x_af3_batched, chi_batched, water_batched)
    
    # Since placeholder_neutron_loss outputs a scalar per sample, loss is shape (5,)
