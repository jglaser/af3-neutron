...
def test_loss_mapping_and_gradients():
    num_af3_atoms = 5
    x_af3_flat = jax.random.normal(jax.random.PRNGKey(0), (1, num_af3_atoms, 3))
    num_oracle_atoms = 7
    ...
    chi_angles = jnp.zeros((1, 2))
    water_rotations = jnp.zeros((1, 0, 3))
...
def test_water_loss_mapping():
    ...
    assert not jnp.isnan(loss)
    assert grad_water.shape == (1, 1, 3)
    assert jnp.any(jnp.abs(grad_water) > 0)
    assert grad_heavy.shape == (1, 1, 3)
